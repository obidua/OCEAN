...
// 1. Read CoreConfig address from .env
// 2. Fetch all contract addresses from CoreConfig
// 3. Update .env with new addresses (replace duplicates)
// 4. Resolve proxy → implementation addresses
// 5. Delete old ABIs in apps/dashboard/store/Contract_ABI
// 6. Fetch & save new ABIs from blockchain explorer
// 7. Generate change reports
//
// Usage: node scripts/sync_coreconfig.js
...
const DASHBOARD_ABI_DIR = ABI_DIRS[1];
...
// Add dashboard .env file for Vite
...
function cleanAbiDirectory(dir) {
  if (!fs.existsSync(dir)) return;
  console.log(`🗑️  Cleaning ABI directory: ${dir}`);
  ...
  console.log(`   Deleted ${deleted} old ABI files`);
}
...
function updateEnvFile(updates) {
  ...
  // Track all occurrences of each key (to handle duplicates)
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (match) {
      ...
      // Keep only the first occurrence in indexMap
      if (!indexMap.has(key)) {
        indexMap.set(key, i);
      }
    }
  ...
  // Remove duplicate lines for keys being updated
  const keysToUpdate = new Set(updates.map(u => u.envKey));
  for (const [key, indices] of keyOccurrences.entries()) {
    if (keysToUpdate.has(key) && indices.length > 1) {
      // Mark duplicate lines for removal (keep only first)
      for (let i = 1; i < indices.length; i++) {
        lines[indices[i]] = null; // Mark for deletion
      }
    }
  }
  ...
  // Remove null entries (duplicates)
  const cleanedLines = lines.filter(line => line !== null);

  fs.writeFileSync(ENV_FILE, cleanedLines.join('\n'), 'utf8');
  return { updated: true, backup: ENV_BACKUP };
}

function createDashboardEnv(updates) {
  console.log('\n📝 Creating dashboard .env file for Vite...');
  const lines = ['# Auto-generated by sync_coreconfig.js', '# Do not edit manually - run: node scripts/sync_coreconfig.js', ''];
  ...
  if (process.env.RPC_URL) {
    lines.push(`VITE_RPC_URL=${process.env.RPC_URL}`);
  }
  
  ensureDir(path.dirname(DASHBOARD_ENV));
  fs.writeFileSync(DASHBOARD_ENV, lines.join('\n') + '\n', 'utf8');
  console.log(`✅ Dashboard .env created at: ${DASHBOARD_ENV}`);
}

async function main() {
  console.log('🚀 Starting automated deployment sync...\n');
  console.log('📋 Workflow:');
  console.log('   1. Read CoreConfig from .env');
  console.log('   2. Fetch all contract addresses from CoreConfig');
  console.log('   3. Resolve proxy → implementation addresses');
  console.log('   4. Clean old ABIs from dashboard');
  console.log('   5. Fetch new ABIs from blockchain explorer');
  console.log('   6. Update .env with new addresses');
  console.log('   7. Create dashboard .env for Vite');
  console.log('   8. Generate change reports\n');
  ...
  // STEP 1: Clean dashboard ABI directory before fetching new ones
  if (ABI_SYNC_ENABLED) {
    cleanAbiDirectory(DASHBOARD_ABI_DIR);
    console.log('');
  }
  ...
  // Always create/update dashboard .env with all contract addresses
  const allAddresses = changeItems
    .filter(item => item.proxyAddress)
    .map(item => ({ envKey: item.envKey, value: item.proxyAddress }));
  
  if (allAddresses.length > 0) {
    createDashboardEnv(allAddresses);
  }
  ...
  console.log('\n✅ ═══════════════════════════════════════════════════════');
  console.log('✅ CoreConfig sync completed successfully!');
  console.log('✅ ═══════════════════════════════════════════════════════\n');
  
  if (envResult.updated) {
    console.log(`📝 Root .env updated (backup: ${ENV_BACKUP})`);
    console.log(`📝 Dashboard .env created: ${DASHBOARD_ENV}`);
  } else {
  ...
  console.log(`   • ${CHANGE_MD}`);
  
  console.log('\n🎯 Next steps:');
  console.log('   1. Review the change report above');
  console.log('   2. Rebuild dashboard: cd apps/dashboard && npm run build');
  console.log('   3. Deploy the new build to your server');
  console.log('   4. Clear browser cache and service worker');
  console.log('\n💡 Tip: Add VITE_ prefix to use env vars in dashboard frontend\n');
}
