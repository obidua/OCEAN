...
import {
  X, Users, Award, Trophy, Gift, Shield, History,
  TrendingUp, Info, Settings, FileDown, LogOut, Presentation, BookOpen, Wallet, Coins,
  AlertTriangle
} from 'lucide-react';
...
const menuSections = [
  {
    ...
    items: [
      { path: '/dashboard/accrued-rewards', label: 'Accrued Rewards', icon: TrendingUp, color: 'text-emerald-400' },
      ...
      { path: '/dashboard/spot-income', label: 'Spot Income', icon: Coins, color: 'text-cyan-300' },
  ...
  },
  {
    ...
    items: [
      ...
      { path: '/dashboard/transaction-history', label: 'Transaction History', icon: History, color: 'text-fuchsia-400' },
    ],
  },
  {
...
export default function MoreMenu({ isOpen, onClose = () => {} }) {
  ...
  useEffect(() => {
    let cancelled = false;
    const resolve = async () => {
      if (!userIdByAdd || !address || address.length !== 42) {
        if (!cancelled) {
      ...
      try {
        const result = await userIdByAdd(address);
        if (!cancelled) {
          const rawId =
